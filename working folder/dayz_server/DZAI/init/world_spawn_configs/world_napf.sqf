...
if ((DZAI_maxHeliPatrols > 0) or {(DZAI_maxLandPatrols > 0)}) then {
	"DZAI_centerMarker" setMarkerPos [10240,10240];
...
if (DZAI_staticAI) then {
	if (DZAI_modName == "unleashed") then {
		...
		//Generic Napf spawns
		[] execVM format ["%1\scripts\setup_autoStaticSpawns.sqf",DZAI_directory]; //IMPORTANT: REMOVE THIS BEFORE ADDING STATIC SPAWNS HERE
	};
};
